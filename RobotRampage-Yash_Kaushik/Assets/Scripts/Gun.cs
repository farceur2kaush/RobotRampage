...
public class Gun : MonoBehaviour {
    ...
    protected void Fire()
    {
        if (ammo.HasAmmo(tag))
        {
            GetComponent<AudioSource>().PlayOneShot(liveFire);      //Normal fire
            ammo.ConsumeAmmo(tag);
        }
        else
        {
            GetComponent<AudioSource>().PlayOneShot(dryFire);       // Empty Gun
        }
    ...
    // checks if the projectile hit the robot or not, if yes then take damage
